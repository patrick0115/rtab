...
class PCSSNode:
    def __init__(self):
        ...
        self.filtered_points_pub = rospy.Publisher('filtered_points', PointCloud2, queue_size=10)
    ...
    def sync_callback(self, image_msg, cloud_msg):
        ...
        try:
            ...
            print("原始點雲數量：", len(transformed_points))  # 印出原始點雲數量

            # filtered_points = [point for point in transformed_points if point[2] > 0 and np.sqrt(point[0]**2 + point[1]**2 + point[2]**2) <= 3]
            filtered_points = [point for point in transformed_points if point[2] > 0 and  (abs(point[0]) < 3 and abs(point[1]) < 3 and abs(point[2]) < 3)]
            # filtered_points = [point for point in transformed_points if point[2] > 0]

            print("過濾後點雲數量：", len(filtered_points))  # 印出過濾後點雲數量
            ...
            if filtered_points:
                ...
                # filtered_cloud_msg = pc2.create_cloud(header, fields, filtered_points)
                # self.filtered_points_pub.publish(filtered_cloud_msg)
            ...
            for i, point in enumerate(projected_points):
                ...
                if 0 <= x < cv_image.shape[1] and 0 <= y < cv_image.shape[0]:
                    ...
                    # 移除alpha通道
                    # rgb = struct.unpack('I', struct.pack('BBB', r, g, b) + struct.pack('B', 0))[0] 
                    rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, 255))[0]
                    colored_point = list(xyz_points[i][:3]) + [rgb]
                    colored_cloud.append(colored_point)
                    # unique_rgb.add((r, g, b))  # 添加RGB值到集合中
                    cv2.circle(cv_image_w_point, (x, y), 3, (0, 255, 255), -1)
                    
            if colored_cloud:
                # print("獨特的RGB值：", unique_rgb)  # 打印出所有獨特的RGB值
                ...
                fields = [
                    ...
                    PointField('y', 4, PointField.FLOAT32, 1),
                    PointField('z', 8, PointField.FLOAT32, 1),
                    PointField('rgb', 12, PointField.UINT32, 1)  # 注意這裡改為 'rgb'
                ]
